<?php
############ Configuration ##############
$thumb_square_size      = 120; //Thumbnails will be cropped to 200x200 pixels
$max_image_size         = 600; //Maximum image size (height and width)
$thumb_prefix           = "thumb_"; //Normal thumb Prefix
$destination_folder     = 'photos/'; //upload directory ends with / (slash)
...
if(isset($_POST) && isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest'){
    ...
    if($image_res){
        ...
        $new_file_name = $image_name_only. '_' .  rand(0, 9999999999) . '-' . time() . '.' . $image_extension;
        ...
        if(normal_resize_image($image_res, $image_save_folder, $image_type, $max_image_size, $image_width, $image_height, $jpeg_quality))
        {
            ...
            // echo '</div>';
            $data = array("success"=>"true", "image"=>$new_file_name);
